...
try {
    $pdo = getDbConnection();
    $stmt = $pdo->prepare("
        SELECT c.id_certificado, c.caminho_arquivo, c.data_emissao, c.codigo_hash,
        ...
        LIMIT 1
    ");
    $stmt->execute([':h'=>$hash, ':uid'=> (int)$_SESSION['id_usuario']]);
...
$projectRoot = __DIR__ . '/../../';
$caminho = trim((string)$row['caminho_arquivo']);
$absPath = $caminho ? $projectRoot . $caminho : '';
$ext = strtolower(pathinfo($caminho, PATHINFO_EXTENSION));

// prefer streaming saved PDF if present
if ($ext === 'pdf' && $absPath && file_exists($absPath)) {
    header('Content-Type: application/pdf');
    header('Content-Disposition: ' . ($download ? 'attachment' : 'inline') . '; filename="' . basename($absPath) . '"');
    header('Content-Length: ' . filesize($absPath));
    readfile($absPath);
    exit;
}

// Attempt TCPDF (if installed)
...
if (file_exists($autoload)) {
    ...
    if (class_exists('TCPDF')) {
        try {
            ...
            if ($caminho && in_array($ext,['jpg','jpeg','png','webp']) && file_exists($absPath)) {
                $pdf->Image($absPath, 0, 0, $pw, $ph, '', '', '', false, 300, '', false, false, 0);
            } else {
            ...
            // set unicode font
            if ($useFont) $pdf->SetFont($useFont,'', max(14,(int)($pw*0.035)));
            else $pdf->SetFont('dejavusans','', max(14,(int)($pw*0.035)));

            $pdf->SetTextColor(15,17,42);
            $pdf->SetXY(0,$y+($h*0.12)); $pdf->Cell($pw,0, $titulo, 0, 1, 'C', 0, '', 0);
            $pdf->SetFont($useFont ?: 'dejavusans','B', max(18,(int)($pw*0.05))); $pdf->SetXY(0,$y+($h*0.36));
            $pdf->Cell($pw,0, $nome, 0, 1, 'C');
            $pdf->SetFont($useFont ?: 'dejavusans','', max(12,(int)($pw*0.024))); $pdf->SetXY(0,$y+($h*0.68));
            $rightText = ($carga ? 'Carga horária: ' . $carga . ' • ' : '') . 'Emitido em: ' . $issued;
            $pdf->Cell($pw,0, $rightText, 0, 1, 'C');

            // footer hash
            $pdf->SetFont($useFont ?: 'dejavusans','', 9);
            $pdf->SetTextColor(100,100,100);
            $pdf->SetXY(20, $ph - 30);
            $pdf->Cell(0,0, 'Código de rastreio: ' . $codigo_hash, 0, 1, 'L');
...
if (extension_loaded('imagick')) {
    try {
        ...
        // draw background image if exists (cover)
        if ($caminho && in_array($ext,['jpg','jpeg','png','webp']) && file_exists($absPath)) {
            $bg = new \Imagick($absPath);
        ...
        $overlay->rectangle($width*0.08, $height*0.58, $width*0.92, $height*0.86);
        $canvas->drawImage($overlay);

        // title
        $draw->setFontSize((int)max(36, $width * 0.034));
        $canvas->annotateImage($draw, $width/2, (int)($height*0.72), 0, $titulo);

        // name (larger)
        $draw->setFontSize((int)max(56, $width * 0.06));
        $draw->setFontWeight(700);
        $canvas->annotateImage($draw, $width/2, (int)($height*0.80), 0, $nome);

        // carga and date
        $draw->setFontSize((int)max(20, $width * 0.024));
        $canvas->annotateImage($draw, $width/2, (int)($height*0.86), 0, ($carga ? 'Carga horária: ' . $carga . ' • ' : '') . 'Emitido em: ' . $issued);
...
<body>
  ...
  <div class="wrap">
    <div class="paper" id="certContainer">
      <?php
        // determine public URL for background image (project under /PPI)
        $bgUrl = '';
        if ($caminho && in_array($ext,['jpg','jpeg','png','webp']) && file_exists($absPath)) {
            $bgUrl = htmlspecialchars('/PPI/' . ltrim($caminho,'/'), ENT_QUOTES, 'UTF-8');
        }
      ?>
      <?php if ($bgUrl): ?>
      ...
      <div class="content">
        <?php if ($bgUrl === ''): ?>
          <!-- optional small logo when no bg -->
          <?php if ($caminho && in_array($ext,['jpg','jpeg','png','webp']) && file_exists($absPath)==false): ?>
            <!-- nothing -->
          <?php endif; ?>
        <?php else: ?>
        ...
        <p class="meta"><?php echo htmlspecialchars(($carga ? 'Carga horária: ' . $carga . ' • ' : '') . 'Emitido em: ' . $issued, ENT_QUOTES, 'UTF-8'); ?></p>
      </div>
